...
class StockDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (torch.tensor(sample['x_dynamic']), 
...
class VariableSelectionNetwork(nn.Module):
    def __init__(self, input_dims, hidden_dim, dropout=0.1):
        super().__init__()
        self.joint_grn = GatedResidualNetwork(input_dims * hidden_dim, hidden_dim, output_dim=input_dims, dropout=dropout)
...
if __name__ == "__main__":
    ...
    # 实例化模型
    model = CompetitionTFT(dynamic_input_dim=5, static_input_dim=2, hidden_dim=64)
    
    # 模拟一次前向传播
    dummy_dynamic = torch.randn(32, 60, 5)
    dummy_static = torch.randn(32, 2)
